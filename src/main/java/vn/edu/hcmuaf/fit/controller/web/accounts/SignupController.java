        ...
        if (!email.equals("") && !pass.equals("") && !re_pass.equals("") && !fname.equals("") && !lname.equals("")) {
            if (!pass.equals(re_pass)) {
                ...
                if (account == null) {
                    ...
                    customerDAO.signup(user.getEmail(), MD5Utils.encrypt( user.getPassword()), user.getFirstName(),user.getLastName(), user.getPhone(), user.getAddress());
                    customerDAO.insert_publicKey(customerDAO.take_id(), publicKeyStr);

                    // luu private key ve may
                    PrintWriter pw = new PrintWriter("C:\\Users\\ADMIN\\Downloads\\privateKey.txt", "UTF-8");
